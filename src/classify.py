...
def identify_doc_type(pdf_name, txt_filepath, page_num=1):
    doc_type = None
    ...
    startswith_map = (
        ...
        ('NOTICE OF ALCOHOL',
         'Notice of Alcohol And Drug Testing Following A Firearms Discharge Incident'),
        ...
        ('DIGITALLY RECORDED DATA',
         'Digitally Recorded Data Viewing/Hold/Duplication Request'),
        ...
        ('A FIREARMS DISCHARGE INCIDENT',
         'Notice of Alcohol And Drug Testing Following a Firearms Discharge Incident'),
        ('DRUG TEST SPECIMEN AFFIDAVIT', 'Drug Test Specimen Affidavit'),
        ('ALTERNATE COLLECTION RECEIPT',
         'Random Drug Testing Unit Alternate Collection Receipt'),
        ...
        ('Data Warehouse Address of Arrest Search',
         'CLEAR Data Warehouse Arrestee Name Check'),
        ('CHICAGO POLICE DEPARTMENT - Digital Mugshot System',
         'Digital Mugshot System Screenshot'),
        ('Investigate Incident - Involved Parties',
         'Investigate Incident Involved Parties'),
        ...
        ("Narcotic & Gang Investigation Section Supplementary Report",
         "Narcotic & Gang Investigation Supplementary Report"),
        ("Incident Check From the Crimes Tables",
         "Incident Check From the Crimes Tables"),
        ...
        ("REQUEST FOR POLICE BOARD REVIEW/ELECT TO FILE A",
         "Request for Police Board Review to File Grievance"),
    ...
    tail_startswith_map = (
        ...
        ('http://mugshot.chicagopolice.local/mod_iad/',
         'Digital Mugshot System Screenshot'),
        ...
        ('https://chris.chicagopolice.org/pls/clear/law_rapsheet_cpd',
    ...
    leftside_maps = (
        ...
        (["DRIVER'S COPY - See Reverse for Court Location",
          "IN THE CIRCUIT COURT OF COOK COUNTY", ], 'Traffic Violation'),
        (['Extension History', 'Current Allegations',
          'Situations (Allegation Details)', 'Status History'], 'Summary Report'),
        (['MEDICAL RECORDS',
          'Spec. Type:',
          'Result Name',
          '*** DRAFT - See performing lab for Final report'], 'medical records'),
        (['EMERGENCY MANAGEMENT & COMMUNICATIONS', 'EXPLANATION FOR MIS GPS 001 REPORT',
          'Explanation for MIS GPS 001 Report'], 'Explanation for MIS GPS 001 Report'),
        (['Legend', 'CPD Vehicle Location', 'Map Created'], 'OEMC GPS Map'),
        (['VALLEY WEST COMMUNITY HOSPITAL', 'DIAGNOSTIC IMAGING DEPARTMENT',
          'Signed Report'], 'Hospital Imaging Report'),
        (['are not intended as complete care. It is important',
          'symptoms become worse or you do not improve as'], 'Emergency Department Discharge Instructions'),
        (['CHICAGO POLICE DEPARTMENT', 'CLEARMAP', 'TOTAL PODS'], 'CLEARMAP POD Map'),
        (['BUREAU OF INTERNAL AFFAIRS', 'INVESTIGATIONS DEVISION', 'Statement Of:',
          'Statement taken at:', 'Questioned by:', 'Date and Time'], 'Statement'),
        (['TASER Information', 'Recorded Firing Data',
          'Recorded X26 Time Changes'], 'TASER Report'),
        (['Case:', 'DEFENDANT NAME', 'ATTORNEY INFORMATION',
          'ENTER=CONT'], 'Court Case Report'),
        (['Map Output', 'Violence & Drug Related Crime', ],
         'Violence & Drug Related Crime Map'),
        (['U.S. Postal Service', 'CERTIFIED MAIL', 'Domestic Mail Only;',
          'For delivery information visit our website'], 'Certified Mail Receipt'),
        (['REASON FOR WORKING OVERTIME', 'OVERTIME AUTHORIZED BY', 'COURT NOTIFICATION RECORD NO',
          'EXPLAIN ASSIGNMENT'], 'Overtime/Compensatory Time Report'),
        (['There is somone to answer',
          'The fullest investigation of this complaint is possible'], 'IPRA Investigation'),
        (['TO REPORT ADDITIONAL INFORMATION', 'COPY OF THE REPORT', 'MAKE THE RIGHT CALL',
          'CHICAGO ALTERNATIVE POLICING'], 'Victim Information Notice'),
        (['PERSONS PRESENT NOT ARRESTED', 'EVIDENCE RECOVERED', 'TOTAL WEIGHT & STREET VALUE',
          'EVIDENCE OFFICER'], 'Narcotic & Gang Investigation Supplementary Report'),
        (['We have nonetheless begun to investigate', 'Please contact me at'],
         'Narcotic & Gang Investigation Supplementary Report'),
        (['HS: Host Send', 'HR: Host Receive',
          'MS: Mailbox Save', 'MP: Mailbox Print'], 'Fax Report'),
    )

    skippable_lines = ['CITY OF CHICAGO']

    with open(txt_filepath, 'r') as fh:
        lines = [l for l in fh.readlines() if re.search(
            '[a-zA-Z]{3,}', l) and l not in skippable_lines]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Classify each pdf page')
    parser.add_argument('pages_repo')
    args = parser.parse_args()
    repo_dir = os.path.join("/in", args.pages_repo)

    data = []
    print('classifying...', flush=True)
    for page_dir in os.listdir(repo_dir):
        parts = page_dir.split('.')
        page_num = parts[-1]
        pdf_name = '.'.join(parts[:-1])
        txt_filepath = os.path.join(repo_dir, page_dir, "page.txt")
        doc_type = identify_doc_type(pdf_name, txt_filepath, page_num)
        print('    %s %s %s' % (pdf_name, page_num, doc_type), flush=True)
        data.append((pdf_name, page_num, '' if doc_type is None else doc_type))

    print('outputing to CSV...', flush=True)
    with open('/out/out.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["pdf_name", "page_num", "page_classification"])

        for pdf_name, page_num, doc_type in data:
            writer.writerow([pdf_name, page_num, doc_type])

    with open('/out/pk', 'w') as pkfile:
        pkfile.write("pdf_name,page_num")
