...
col_based_headers = {
    # incident details is horizontally oriented AND columnar. breaks code for the moment, so this is a placeholder.,
...
def find_label(label_txt, tokens):
    ...
    for block_num, tokens in block_tokens.items():
        block_lines = dict([((t['line_num'], t['par_num']), [])
                            for t in tokens])
        for token in tokens:
...
def last_column_header_bounds(tokens, col_key, threshold=.4, label_name=None):
    ...
    for p1 in range(len(highest_toks)):
        ...
        for p2 in range(len(highest_toks)):
            ...
            potential_tokens_combined.append(
                highest_toks[p1] + highest_toks[p2])
...
def parse_tsv_data(tsv_path, pdf_name, pdf_page):
    tokens, tsv_fieldnames = tsv_data(tsv_path)
    ...
    label_boundaries = [(i, find_label(i, tokens))
                        for i in col_based_headers.keys()]
    label_boundaries = [(l, b) for l, b in label_boundaries if b[2]]

    section_tops = sorted([bounds[2]
                           for key, bounds in label_boundaries if bounds[2]])
    ...
    for label, top, bottom in section_boundaries:
        ...
        section_tokens = tokens_in_bounds(
            min_left, page_width, label_bottom, bottom, tokens)
        ...
        for col_name in col_based_headers[label][::-1]:
            header_bounds = last_column_header_bounds(
                section_tokens, col_name, label_name=label)
            ...
            column_tokens = tokens_in_bounds(
                header_left-3, prev_left + 3, header_row_bottom+8, bottom, section_tokens)
            ...
            section_tokens = tokens_in_bounds(
                0, header_left, label_bottom, bottom, tokens)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Parse summary data')
    ...
    for page_dir in os.listdir(repo_dir):
        ...
        tsv_path = os.path.join(repo_dir, page_dir, "page.tsv")

        sections = parse_tsv_data(tsv_path, pdf_name, page_num)
        print('    %s %s %s' % (pdf_name, page_num, sections), flush=True)
        data.append((pdf_name, page_num, '' if sections is None else sections))

    print('outputing to CSV...', flush=True)
    with open('/out/out.csv', 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["pdf_name", "page_num", "sections"])

        for pdf_name, page_num, sections in data:
            writer.writerow([pdf_name, page_num, sections])
