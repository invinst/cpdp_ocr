...
#error_counts, as needed
error_counts = {'warning_pdf':0, 'good_page':0, 'bad_page':0, 'bad_pdf': 0}

PDFSPATH = '/opt/data/cpdp_pdfs/ocrd'
...
def identify_pdf_cpd_ids(pdf_name, begin, end):
    ...
    pages_cpd_ids = []
    pdf_dir = '{}/{}'.format(PDFSPATH, pdf_name)
...
def identify_doc_type(pdf_name, page_num=1, filename=None):
    doc_type = None
    filepath = '{}/{}'.format(PDFSPATH, pdf_name)
    ...
    #If the filename column exists in cr_ranges.csv, check if 
    if filename:
        doc_type = re.sub('^.*\.CR', '', filename)
        doc_type = re.sub('\.[pP][dD][fF]$', '', doc_type).strip()

        if 'DOCID' in doc_type:
            doc_type = None
        else:
            return doc_type
...
def tag_pdf(pdf_name, begin, end, log_id, filename):
    ...
    for txt_file in txt_files:
        ...
        tagged_pages.append({'pdf_name': pdf_name, 'page_num': 
            page_num, 'doc_type': doc_type, 'cr_id': cr_id})
